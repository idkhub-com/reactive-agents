  ...
  healthCheck(): {
    ...
    try {
      ...
      const tables = this.db
        ...
        WHERE type='table' AND name IN ('runs', 'vehicles', 'analyses')
      `)
        .all() as Array<{ name: string }>;

      const expectedTables = ['runs', 'vehicles', 'analyses'];
  ...
  getDatabaseStats(): {
    ...
    try {
      const totalRuns = this.db
        ...
        .get() as { count: number };
      const totalVehicles = this.db
        .prepare('SELECT COUNT(*) as count FROM vehicles')
        .get() as { count: number };
      const totalAnalyses = this.db
      ...
      this.logger.error({ err: error }, 'Failed to get database stats');
      return {
        totalRuns: 0,
        totalVehicles: 0,
        totalAnalyses: 0,
