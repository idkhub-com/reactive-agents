...
/**
 * Super Agents Unified AI Workflow Example
 *
 * This example demonstrates both basic chat completion and complex multi-node AI agent workflows.
 * It shows how Super Agents can handle simple requests and sophisticated enterprise workflows.
 *
 * IMPROVEMENTS & FIXES:
...
// Configuration
const SA_URL = 'http://localhost:3000/v1';
const AUTH_TOKEN = 'super-agents';
...
async function researchAnalysisWorkflow(userQuery: string): Promise<void> {
  ...
  const analysisConfig: SuperAgentsConfig = {
  ...
  const synthesisConfig: SuperAgentsConfig = {
  ...
  const reviewConfig: SuperAgentsConfig = {
...
async function strategicPlanningWorkflow(planningGoal: string): Promise<void> {
  ...
  const planningConfig: SuperAgentsConfig = {
  ...
  const detailConfig: SuperAgentsConfig = {
  ...
  const riskConfig: SuperAgentsConfig = {
...
async function contentCreationWorkflow(contentBrief: string): Promise<void> {
  ...
  const ideationConfig: SuperAgentsConfig = {
  ...
  const developmentConfig: SuperAgentsConfig = {
...
async function runUnifiedExample(runInParallel = false): Promise<void> {
  console.log('Super Agents Unified AI Workflow Example');
  console.log('==================================\n');
