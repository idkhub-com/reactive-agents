...
export interface Skill {
  ...
  seed_system_prompt: string | null;
...
/** The models attached to a skill: what its arms are generated for. */
export const getSkillModels = async (
  request: APIRequestContext,
  skillId: string,
): Promise<{ id: string; model_name: string }[]> => {
  const response = await request.get(`${SKILLS_PATH}/${skillId}/models`);
  if (response.status() !== 200) {
    throw new Error(
      `GET ${SKILLS_PATH}/${skillId}/models -> ${response.status()} ${await response.text()}`,
    );
  }
  return response.json() as Promise<{ id: string; model_name: string }[]>;
};

/** The evaluations scoring a skill's responses, generated by the optimizer. */
export const getSkillEvaluations = async (
  request: APIRequestContext,
  skillId: string,
): Promise<{ id: string; evaluation_method: string }[]> => {
  const response = await request.get(`${SKILLS_PATH}/${skillId}/evaluations`);
  if (response.status() !== 200) {
    throw new Error(
      `GET ${SKILLS_PATH}/${skillId}/evaluations -> ${response.status()} ${await response.text()}`,
    );
  }
  return response.json() as Promise<
    { id: string; evaluation_method: string }[]
  >;
};
