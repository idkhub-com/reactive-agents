...
// Mock dependencies
vi.mock('@client/api/v1/idk/observability/logs', (): object => ({
  queryLogs: vi.fn().mockResolvedValue([
    {
      ...
      ai_provider_request_log: {
        response_body: {
          choices: [
            {
              message: { role: ChatCompletionMessageRole.USER, content: 'foo' },
              index: 0,
              finish_reason: 'stop',
            },
          ],
...
vi.mock('@client/api/v1/idk/agents', (): object => ({
  getAgents: vi.fn().mockResolvedValue([]),
  createAgent: vi.fn(),
  updateAgent: vi.fn(),
  deleteAgent: vi.fn(),
}));
vi.mock('@client/hooks/use-toast', (): object => ({
  useToast: (): { toast: () => void } => ({ toast: vi.fn() }),
}));
vi.mock('@shared/types/data/log', (): object => ({
  LogsQueryParams: { parse: (): Record<string, never> => ({}) },
}));
