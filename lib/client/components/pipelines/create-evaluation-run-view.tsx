...
export function CreateEvaluationView(): ReactElement {
  ...
  const handleBack = () => {
    switch (currentStep) {
      case 'select-dataset': {
        ...
        const fallbackUrl = `/pipelines/${encodeURIComponent(selectedAgent.name)}/${encodeURIComponent(selectedSkill.name)}/evaluations`;
      ...
      default: {
        ...
        const defaultFallbackUrl = `/pipelines/${encodeURIComponent(selectedAgent.name)}/${encodeURIComponent(selectedSkill.name)}/evaluations`;
