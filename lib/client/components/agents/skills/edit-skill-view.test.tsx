...
import { EditSkillView } from './edit-skill-view';
...
describe('EditSkillView', () => {
  ...
  describe('Rendering', () => {
    ...
    it('displays skill name as read-only', () => {
      renderEditSkillView();
      expect(screen.getByText('Skill Name (Read-only)')).toBeInTheDocument();
  ...
  describe('Form Validation', () => {
    it('has max configurations field with correct constraints', () => {
      ...
      expect(maxConfigField).toHaveAttribute('min', '1');
      expect(maxConfigField).toHaveAttribute('max', '100');
    ...
    it('allows valid input in max configurations field', () => {
      ...
      // Test valid values
      fireEvent.change(maxConfigField, { target: { value: '50' } });
      expect(maxConfigField).toHaveValue(50);

      fireEvent.change(maxConfigField, { target: { value: '1' } });
      expect(maxConfigField).toHaveValue(1);

      fireEvent.change(maxConfigField, { target: { value: '100' } });
      expect(maxConfigField).toHaveValue(100);
    });
  ...
  describe('Accessibility', () => {
    ...
    it('has proper form descriptions', () => {
      ...
      expect(
        screen.getByText(/provide additional context about the skill/i),
      ).toBeInTheDocument();
      expect(
