import type { OpenAIResponse } from '@server/types/model-response';
...
export const OpenAICreateModelResponseConfig: AIProviderFunctionConfig = {
  input: {
    param: 'input',
    required: true,
  },
...
export function* openAIModelResponseJSONToStreamGenerator(
  ...
  if (openAIResponse.error?.code) {
    yield getResponseErrorEvent(openAIResponse.error);
    return;
  }

  if (openAIResponse.status === 'failed') {
    yield getResponseFailedEvent(openAIResponse);
    return;
  }

  if (openAIResponse.status === 'incomplete') {
    yield getResponseIncompleteEvent(openAIResponse);
    return;
  }

  const responseId = getRandomId();
  yield getResponseCreatedEvent(openAIResponse, responseId);
  yield getResponseInProgressEvent(openAIResponse, responseId);
  for (const [index, outputItem] of openAIResponse.output.entries()) {
  ...
  yield getResponseCompletedEvent(openAIResponse, responseId);
}
