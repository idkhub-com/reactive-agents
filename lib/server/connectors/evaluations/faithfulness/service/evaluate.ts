...
import { getFaithfulnessTemplate } from '../templates/main';

function pickFaithfulnessData(
  log: Log,
  params: FaithfulnessEvaluationParameters,
): {
  context: string;
  answer: string;
  ...
  console.log(`Faithfulness - Extracted data for log ${log.id}:`, {
...
async function evaluateSingleLog(
  ...
  try {
    const { context, answer, retrieval_context } = pickFaithfulnessData(
      log,
      params,
    );
    ...
    console.log(
      `Faithfulness - Evaluating log ${log.id} with context: "${context.substring(0, 50)}..." and answer: "${answer.substring(0, 50)}..."`,
    );

    const tpl = getFaithfulnessTemplate({
      context,
      answer,
      retrieval_context,
    ...
    console.log(`Faithfulness - Judge result for log ${log.id}:`, {
    ...
    console.error(`Faithfulness - Error evaluating log ${log.id}:`, error);

    if (retryCount < maxRetries) {
      console.log(
        `Faithfulness - Retrying evaluation for log ${log.id} (attempt ${retryCount + 1})`,
      );
...
async function processLogsInBatches(
  ...
  for (let i = 0; i < logs.length; i += batch_size) {
    ...
    console.log(
      `Faithfulness - Processed batch ${Math.floor(i / batch_size) + 1}/${Math.ceil(logs.length / batch_size)}`,
    );
  }
...
export async function evaluateFaithfulness(
  ...
  console.log(`Faithfulness - Starting evaluation for ${logs.length} logs`);
  ...
  const evaluationRunParams: EvaluationRunCreateParams = {
    ...
    evaluation_method: EvaluationMethodName.FAITHFULNESS,
    name: evalRunOptions.name || 'Faithfulness Evaluation',
    description:
      evalRunOptions.description ||
      'Evaluating faithfulness of answers to context',
    metadata: {
      parameters: params,
  ...
  try {
    ...
    console.log(
      `Faithfulness - Evaluation completed with average score: ${averageScore}`,
    );

    return { averageResult, evaluationRun };
  } catch (error) {
    console.error('Faithfulness - Error in evaluation:', error);
    await userDataStorageConnector.updateEvaluationRun(evaluationRun.id, {
...
export async function evaluateOneLogForFaithfulness(
  evaluationRunId: string,
  log: IdkRequestLog,
  userDataStorageConnector: UserDataStorageConnector,
): Promise<void> {
  console.log(
    `Faithfulness - Starting evaluation for log ${log.id} in run ${evaluationRunId}`,
  );
  ...
  if (!evaluationRun) {
    console.error(`Faithfulness - Evaluation run ${evaluationRunId} not found`);
    throw new Error(`Evaluation run ${evaluationRunId} not found`);
  }

  console.log(
    `Faithfulness - Found evaluation run ${evaluationRunId} with method ${evaluationRun.evaluation_method}`,
  );

  let params: FaithfulnessEvaluationParameters;
  try {
    params = FaithfulnessEvaluationParameters.parse(
      evaluationRun.metadata?.parameters || {},
    );
    console.log(`Faithfulness - Parsed parameters:`, {
    ...
    console.error(
      `Faithfulness - Error parsing parameters for run ${evaluationRunId}:`,
      error,
    );
