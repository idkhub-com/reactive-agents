...
import { SkillDashboardView } from '@client/components/pipelines';
  ...
  useEffect(() => {
    if (!isLoadingFromStorage) {
      if (!navigationState.selectedAgent) {
        router.push('/pipelines?error=agent-not-found');
        return;
      }
      if (!navigationState.selectedSkill) {
        router.push(
          `/pipelines/${encodeURIComponent(navigationState.selectedAgent.name)}?error=skill-not-found`,
        );
        return;
      }
    }
