import { getBoundaryFromContentType } from '@api/handlers/stream-handler-utils';
    ...
    if (!aws_s3_bucket || !aws_bedrock_model) {
      return new Response(
        JSON.stringify({
          status: 'failure',
          message:
            'Please make sure you have set the aws s3 bucket and aws bedrock models in config',
        }),
        {
          status: 400,
          headers: {
