...
vi.mock('@api/handlers/handler-utils', () => ({
...
const { embeddingsRouter } = await import('@api/v1/embeddings');
...
const app = new Hono<AppEnv>()
  .use('*', async (c, next) => {
    ...
    mockContext.set('sa_request_data', {
      function_name: 'embed',
      requestBody: {
...
describe('Embeddings API', () => {
  ...
  describe('POST /', () => {
    ...
    it('should pass correct parameters to tryTargets', async () => {
      ...
      const [context, saConfig, saRequestData] = mockTryTargets.mock.calls[0];
      expect(context).toBeDefined();
      expect(saConfig).toMatchObject({
      ...
      expect(saRequestData).toMatchObject({
        function_name: 'embed',
        requestBody: {
          model: 'text-embedding-3-small',
          input: 'test input',
