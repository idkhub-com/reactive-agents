...
export async function getModels(params?: ModelQueryParams): Promise<Model[]> {
  const response = await client.v1['super-agents'].models.$get({
    query: {
      id: params?.id,
...
export async function getModelById(id: string): Promise<Model> {
  const response = await client.v1['super-agents'].models[':id'].$get({
    param: {
      id,
    },
  });
...
export async function createModel(model: ModelCreateParams): Promise<Model> {
  const response = await client.v1['super-agents'].models.$post({
    json: model,
  });
...
export async function updateModel(
  ...
  const response = await client.v1['super-agents'].models[':id'].$patch({
    param: {
      id,
    },
    json: update,
...
export async function deleteModel(id: string): Promise<void> {
  const response = await client.v1['super-agents'].models[':id'].$delete({
    param: {
      id,
    },
  });
