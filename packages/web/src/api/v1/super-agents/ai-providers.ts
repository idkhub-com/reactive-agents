...
export async function getAIProviderSchemas(): Promise<AIProviderSchemasResponse> {
  const response =
    await client.v1['super-agents']['ai-providers'].schemas.$get();
...
export async function getAIProviderSchema(
  ...
  const response = await client.v1['super-agents']['ai-providers'].schemas[
    ':provider'
  ].$get({
    param: { provider },
  });
...
export async function getAIProviderAPIKeys(
  ...
  const response = await client.v1['super-agents']['ai-providers'].$get({
    query: {
      id: params.id,
...
export async function createAIProvider(
  ...
  const response = await client.v1['super-agents']['ai-providers'].$post({
    json: params,
  });
...
export async function updateAIProvider(
  ...
  const response = await client.v1['super-agents']['ai-providers'][
    ':id'
  ].$patch({
    param: {
      id: id,
