...
import { createResponse } from '@server/utils/reactive-agents/responses';
import {
...
describe('createResponse', () => {
  ...
  beforeEach(() => {
    ...
    // Mock request data
    const mockReactiveAgentsRequestData: ChatCompletionRequestData = {
    ...
    // Mock options
    mockOptions = {
      raRequestData: mockReactiveAgentsRequestData,
    ...
    it('should process successful response correctly', async () => {
      ...
      expect(mockContext.set).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          status: 200,
          request_body: mockOptions.raRequestData.requestBody,
          response_body: {
            choices: [{ message: { content: 'Hello there!' } }],
          },
          raw_request_body: JSON.stringify(
            mockOptions.raRequestData.requestBody,
          ),
          raw_response_body:
            '{"choices":[{"message":{"content":"Hello there!"}}]}',
          cache_status: 'miss',
